...
def get_metrics(y_true, y_pred, folder_name='metrics', prompt=''):
    ...
    label_metrics, micro, macro = LabelMetrics(labels).calculate_label_mentrics(y_true, y_pred)
    ...
    other_metrics_df = pd.DataFrame([['propensity', propensity_f, propensity_precision, propensity_recall], ['micro', micro[0], micro[1], micro[2]], ['macro', macro[0], macro[1], macro[2]]], columns=['metric', 'precision', 'recall', 'f1'])
